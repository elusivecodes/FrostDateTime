...
(function (global, factory) {
  ...
  function () {
    ...
    _createClass(DateInterval, [{
      ...
      value: function format(formatString) {
        ...
        var output = '';
        var prefixed = false;
        formatString.split('').forEach(function (char) {
          if (!prefixed && char === '%') {
            prefixed = true;
            return;
          }
          ...
          output += DateInterval.formatData[char](_this);
        });
        return output;
      }
      /**
      ...
      value: function toString() {
        ...
        return formats.length > 0 ? DateInterval.lang.relative[this.invert ? 'ago' : 'in'].replace('{interval}', this.format(formats.map(function (f) {
      ...
      value: function fromString(time) {
        ...
        var regex = new RegExp(DateInterval.stringRegex, 'gi');
        var match;
  ...
  DateInterval.isoRegex = /^P(?:(\d+)Y)?(?:(\d+)M)?(?:(\d+)D)?(?:(\d+)W)?(?:T(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?|)$/; // string Regex
  ...
  DateTime.formatData = {
    ...
    S: {
      regex: function regex() {
        return '(' + DateTime.lang.ordinal.join('|') + ')';
      },
      output: function output(datetime) {
    ...
    a: {
      value: 'pm',
      regex: function regex() {
        return '(' + DateTime.lang.ampm.lower.join('|') + ')';
      },
      input: function input(value) {
        return DateTime.lang.ampm.lower.findIndex(function (period) {
          return period === value;
        });
      },
      output: function output(datetime) {
        return datetime.getHours() < 12 ? DateTime.lang.ampm.lower[0] : DateTime.lang.ampm.lower[1];
      }
    },
    // day period upper
    A: {
      value: 'pm',
      regex: function regex() {
        return '(' + DateTime.lang.ampm.upper.join('|') + ')';
      },
      input: function input(value) {
        return DateTime.lang.ampm.upper.findIndex(function (period) {
          return period === value;
        });
      },
      output: function output(datetime) {
        return datetime.getHours() < 12 ? DateTime.lang.ampm.upper[0] : DateTime.lang.ampm.upper[1];
      }
    },
    // swatch time
    B: {
    ...
    O: {
      ...
      output: function output(datetime) {
        return (datetime.offset > 0 ? '-' : '+') + DateTime._formatNumber(Math.abs(Math.floor(datetime.offset / 60)), 2) + DateTime._formatNumber(datetime.offset % 60, 2);
      }
    },
    // offset colon
    P: {
    ...
    Z: {
      ...
      output: function output(datetime) {
        return DateTime._formatNumber(datetime.offset * -60);
      }
    },
    ...
    '#': {
      regex: function regex() {
        return '([' + DateTime.seperators.map(function (seperator) {
          return '\\' + seperator;
        }).join('') + '])';
      }
    },
  ...
  Object.assign(DateTime.prototype, {
    ...
    getBeat: function getBeat() {
      ...
      return Math.floor((tempDate.getUTCHours() * 3600000 + tempDate.getUTCMinutes() * 60000 + tempDate.getUTCSeconds() * 1000 + tempDate.getUTCMilliseconds()) / 86400);
    },
    ...
    getISOWeek: function getISOWeek() {
      var eek = DateTime._isoDate(this.getYear(), this.getMonth(), this.getDate());

      var firstWeek = DateTime._isoDate(eek.getUTCFullYear(), 0, 4);
    ...
    getTimezoneAbbr: function getTimezoneAbbr() {
      return this.isDST() ? this.transition.dst : this.transition.abbr;
    },
  ...
  Object.assign(DateTime.prototype, {
    ...
    format: function format(formatString) {
      ...
      formatString.split('').forEach(function (char) {
        ...
        output += DateTime.formatData[char].output(_this2);
      });
      return output;
    },
     ...
     * @returns {string} The formatted string of the current DateTime
     */
    toLocaleDateString: function toLocaleDateString(locales) {
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
      return this.utcDate.toLocaleDateString(locales || DateTime.defaultLocale, _objectSpread({
     ...
     * @returns {string} The formatted string of the current DateTime
     */
    toLocaleString: function toLocaleString(locales) {
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
      return this.utcDate.toLocaleString(locales || DateTime.defaultLocale, _objectSpread({
        timeZone: this.timezone
      }, options));
    },
     ...
     * @returns {string} The formatted string of the current DateTime
     */
    toLocaleTimeString: function toLocaleTimeString(locales) {
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
      return this.utcDate.toLocaleTimeString(locales || DateTime.defaultLocale, _objectSpread({
        timeZone: this.timezone
      }, options));
    },
  ...
  Object.assign(DateTime.prototype, {
    ...
    _checkOffset: function _checkOffset() {
      this.offset = this.timezone === 'UTC' ? 0 : (new Date(DateTime.utcFormatter.format(this)) - new Date(this.formatter.format(this))) / 60000;
    },
    ...
    _getTransition: function _getTransition() {
      ...
      this.transition = DateTime.timezones[this.timezone].find(function (transition) {
    ...
    _makeFormatter: function _makeFormatter() {
      this.formatter = new Intl.DateTimeFormat(DateTime.formatterLocale, _objectSpread({}, DateTime.formatterOptions, {
        timeZone: this.timezone
      }));
    },
  ...
  Object.assign(DateTime.prototype, {
    ...
    diff: function diff(other) {
      ...
      var tempDate = new DateTime(other, this.timezone);
      ...
      var thisMinute = this.getMinutes();
      var thisSecond = this.getSeconds();
      var thisMillisecond = this.getMilliseconds() * 1000;
      var otherMonth = tempDate.getMonth();
      var otherDate = tempDate.getDate();
      var otherHour = tempDate.getHours();
      var otherMinute = tempDate.getMinutes();
      var otherSecond = tempDate.getSeconds();
      var otherMillisecond = tempDate.getMilliseconds() * 1000;
    ...
    isDST: function isDST() {
      ...
      var dateA = new DateTime([year, 0, 1], this.timezone);
      var dateB = new DateTime([year, 5, 1], this.timezone);
  ...
  Object.assign(DateTime, {
     ...
     * @returns {DateTime} The new DateTime object
     */
    fromFormat: function fromFormat(dateString, formatString) {
      var _this3 = this;

      var data = {};
      formatString.split('').forEach(function (token) {
        if (_this3.seperators.includes(token)) {
          dateString = dateString.substring(1);
          return;
        }

        if (!_this3.formatData[token] || !_this3.formatData[token].regex) {
          throw new Error("Invalid token in DateTime format: ".concat(token));
        }

        var regex = _this3.formatData[token].regex;
        var regexp = typeof regex === 'function' ? regex(token) : regex;
        var dateMatch = dateString.match(new RegExp('^' + regexp));

        if (!dateMatch) {
          throw new Error("Unmatched token in DateTime string: ".concat(token));
        }
        ...
        if (['!', '|'].includes(token)) {
          ...
          Object.assign(data, token === '!' ? epoch : _objectSpread({}, epoch, data));
        } else {
          if (!_this3.formatData[token].input) {
            return;
          }

          var value = _this3.formatData[token].value;
          data[value] = _this3.formatData[token].input(dateMatch[1]);
        }
      });
    ...
    fromObject: function fromObject(dateObject) {
      ...
      if (dateObject.timestamp) {
        ...
        date = [newDate.year, newDate.month, newDate.date, newDate.hours, newDate.minutes, newDate.seconds, newDate.milliseconds];
      }
      ...
      if (dateObject.hasOwnProperty('timezone')) {
        ...
        timezone = this.timezoneFromAbbrOffset(date, dateObject.hasOwnProperty('timezoneAbbr') ? dateObject.timezoneAbbr : null, dateObject.hasOwnProperty('offset') ? dateObject.offset : null);
      }
  ...
  Object.assign(DateTime, {
    ...
    _isoDate: function _isoDate() {
      ...
      var day = this._isoDay(date.getUTCDay());
  ...
  Object.assign(DateTime, {
    ...
    daysInMonth: function daysInMonth(year, month) {
      ...
      return this.monthDays[month] + (month == 1 && this.isLeapYear(date.getUTCFullYear()) ? 1 : 0);
    },
    ...
    timezoneFromAbbrOffset: function timezoneFromAbbrOffset(date) {
      ...
      return Object.keys(this.timezones).find(function (timezone) {
  ...
  var zones = {
    ...
    "Asia/Tokyo": 221,
    "Asia/Tomsk": 222,
    "Asia/Ulaanbaatar": 223,
    "Asia/Urumqi": 224,
    "Asia/Ust-Nera": 225,
    "Asia/Vientiane": 169,
    "Asia/Vladivostok": 226,
    "Asia/Yakutsk": 227,
    "Asia/Yangon": 228,
    "Asia/Yekaterinburg": 229,
    "Asia/Yerevan": 230,
    "Atlantic/Azores": 231,
    "Atlantic/Bermuda": 232,
    "Atlantic/Canary": 233,
    "Atlantic/Cape_Verde": 234,
    "Atlantic/Faroe": 235,
    "Atlantic/Madeira": 236,
    "Atlantic/Reykjavik": 237,
    "Atlantic/South_Georgia": 238,
    "Atlantic/St_Helena": 0,
    "Atlantic/Stanley": 239,
    "Australia/Adelaide": 240,
    "Australia/Brisbane": 241,
    "Australia/Broken_Hill": 240,
    "Australia/Currie": 241,
    "Australia/Darwin": 240,
    "Australia/Eucla": 242,
    "Australia/Hobart": 241,
    "Australia/Lindeman": 241,
    "Australia/Lord_Howe": 243,
    "Australia/Melbourne": 241,
    "Australia/Perth": 244,
    "Australia/Sydney": 241,
    "Europe/Amsterdam": 245,
    "Europe/Andorra": 246,
    "Europe/Astrakhan": 247,
    "Europe/Athens": 248,
    "Europe/Belgrade": 157,
    "Europe/Berlin": 249,
    "Europe/Bratislava": 250,
    "Europe/Brussels": 251,
    "Europe/Bucharest": 252,
    "Europe/Budapest": 157,
    "Europe/Busingen": 253,
    "Europe/Chisinau": 254,
    "Europe/Copenhagen": 255,
    "Europe/Dublin": 256,
    "Europe/Gibraltar": 257,
    "Europe/Guernsey": 258,
    "Europe/Helsinki": 259,
    "Europe/Isle_of_Man": 258,
    "Europe/Istanbul": 260,
    "Europe/Jersey": 258,
    "Europe/Kaliningrad": 261,
    "Europe/Kiev": 262,
    "Europe/Kirov": 263,
    "Europe/Lisbon": 264,
    "Europe/Ljubljana": 157,
    "Europe/London": 258,
    "Europe/Luxembourg": 265,
    "Europe/Madrid": 266,
    "Europe/Malta": 157,
    "Europe/Mariehamn": 259,
    "Europe/Minsk": 267,
    "Europe/Monaco": 268,
    "Europe/Moscow": 269,
    "Europe/Oslo": 157,
    ...
    "Europe/Rome": 272,
    "Europe/Samara": 273,
    "Europe/San_Marino": 272,
    "Europe/Sarajevo": 157,
    "Europe/Saratov": 274,
    "Europe/Simferopol": 275,
    "Europe/Skopje": 157,
    "Europe/Sofia": 276,
    "Europe/Stockholm": 277,
    "Europe/Tallinn": 278,
    "Europe/Tirane": 279,
    "Europe/Ulyanovsk": 280,
    "Europe/Uzhgorod": 281,
    "Europe/Vaduz": 253,
    "Europe/Vatican": 272,
    "Europe/Vienna": 157,
    "Europe/Vilnius": 282,
    "Europe/Volgograd": 283,
    "Europe/Warsaw": 284,
    "Europe/Zagreb": 157,
  ...
  DateTime.seperators = [';', ':', '/', '.', ',', '-', '(', ')']; // days in months

  DateTime.monthDays = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]; // default formats
  ...
  DateTime.defaultLocale = Intl.DateTimeFormat().resolvedOptions().locale; // default timezone

  DateTime.defaultTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone; // UTC Formatter
  ...
  DateTime.utcFormatter = new Intl.DateTimeFormat(DateTime.formatterLocale, DateTime.formatterOptions); // aliases

  DateTime.prototype.getFullYear = DateTime.prototype.getYear;
  DateTime.prototype.setFullYear = DateTime.prototype.setYear; // timezones

  DateTime.timezones = {};
  Object.keys(zones).forEach(function (timezone) {
    var parts = values[zones[timezone]].split('|');
    var abbr = parts.shift().split(';').map(function (a) {
      return a || 'LMT';
    });
    var transitions = parts.shift().split(';').map(function (t) {
      var data = t.split(',');
      data[0] = data[0] ? parseInt(data[0], 36) : -Number.MAX_VALUE;
      return data;
    });
    DateTime.timezones[timezone] = transitions.map(function (transition, i) {
      return {
        start: transition[0],
